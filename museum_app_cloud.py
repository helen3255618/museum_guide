...
# ── JS audio queue — injected once at page load ──────────────
# Browser-side queue: receives base64 mp3 chunks, plays them
# strictly in order, one at a time. Survives Streamlit rerenders.
st.markdown("""
<script>
window._audioQueue = window._audioQueue || [];
window._audioPlaying = window._audioPlaying || false;
...
window._playNext = function() {
    ...
    audio.onended = window._playNext;
    audio.onerror = window._playNext;
    audio.play().catch(window._playNext);
};
</script>
""", unsafe_allow_html=True)

def push_audio_to_queue(audio_bytes: bytes):
    """Push one mp3 chunk into the browser-side JS queue."""
    b64 = base64.b64encode(audio_bytes).decode()
    st.markdown(
        f'<script>window.enqueueAudio("{b64}");</script>',
        unsafe_allow_html=True
    )
...
def stream_and_speak(messages: list) -> str:
    """
    Stream GPT sentence by sentence.
    Each complete sentence → TTS immediately → pushed to JS queue.
    JS queue plays them strictly in order, one at a time.
    """
    ...
    for chunk in stream:
        ...
        # When sentence complete → TTS → enqueue
        ...
        if len(parts) > 1:
            for sentence in parts[:-1]:
                ...
                if len(s) > 8:
                    audio = tts(s)
                    push_audio_to_queue(audio)
            buffer = parts[-1]

    # Remaining text
    if buffer.strip() and len(buffer.strip()) > 4:
        audio = tts(buffer.strip())
        push_audio_to_queue(audio)
