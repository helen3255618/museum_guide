...
def stream_response_with_tts(messages: list):
    ...
    for chunk in stream:
        ...
        if len(parts) > 1:
            for sentence in parts[:-1]:
                s = sentence.strip()
                if len(s) > 8:
                    play_audio(tts_chunk(s))
            buffer = parts[-1]

    if buffer.strip() and len(buffer.strip()) > 4:
        play_audio(tts_chunk(buffer.strip()))
...
if audio_input:
    ...
    if user_text:
        ...
        reply = stream_response_with_tts(messages)
