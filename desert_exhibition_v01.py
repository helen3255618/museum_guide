...
def load_knowledge_base():
    ...
    Rebuilds automatically if collection exists but is empty (e.g. interrupted first run).
    """
    ...
    if "exhibition" in existing:
        collection = chroma_client.get_collection("exhibition")
        if collection.count() > 0:
            return collection
        # Collection exists but is empty — delete and rebuild
        chroma_client.delete_collection("exhibition")
    ...
    if not chunks:
        return collection  # JSONL files not found — collection stays empty
...
# ── Exhibit identification via GPT-4o (Plan C) ───────────────
...
# ── Streaming answer with RAG context ────────────────────────
...
with st.sidebar:
    ...
    if kb_count > 0:
        st.markdown(
            ...
            f'color:#7a9a6a;letter-spacing:0.1em;">✓ KB: {kb_count} chunks loaded</p>',
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            '<p style="font-family:JetBrains Mono,monospace;font-size:0.6rem;'
            'color:#c47a6a;letter-spacing:0.1em;">✗ KB: no chunks — check JSONL files</p>',
            unsafe_allow_html=True
        )
    st.divider()
    ...
    VERSIONS = [
        {
            ...
            "date": "2026-03-26",
            "changes": [
                "🔧 KB auto-rebuild if collection exists but is empty",
                "📊 Sidebar KB status indicator (chunk count)",
                "🔍 Retrieved chunks debug panel with zone/domain/subject",
                "🗂 Step 1 / Step 2 input layout — linear flow, full-width mic",
                "✕ Clear name/photo inline — no separate row",
                "⏸ Pause tucked under mic",
            ],
        },
        {
            "version": "v4.0",
...
# Step 1 row: scientific name | or | camera
...
if audio_input:
    ...
    if user_text:
        ...
        # If no photo (or low-confidence), fall back to manual specimen name
