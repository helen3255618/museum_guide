...
# ── Iframe microphone permission fix (iOS compatible) ─────────
st.components.v1.html("""
    <script>
        // Set allow attribute on parent iframe (desktop)
        const iframe = window.frameElement;
        if (iframe) {
            iframe.allow = "microphone; camera";
            iframe.setAttribute("allow", "microphone; camera");
        }
        // Walk up all parent frames (iOS requires this)
        try {
            let win = window;
    ...
    VERSIONS = [
        {
            ...
            "date": "2026-03-23",
            "changes": [
                "📱 iOS fix: replaced st.audio_input with audiorecorder (iOS Safari compatible)",
                "📱 Improved iOS microphone permission fix — walks up all parent iframes",
...
if len(audio_input) > 0:
    with st.spinner("Transcribing..."):
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
            audio_input.export(f.name, format="wav")
            tmp_path = f.name
        try:
    ...
    if user_text:
        ...
        st.markdown(
            f'<div class="label-visitor">Visitor</div>'
            f'<div class="msg-visitor">{display_text}</div>',
            unsafe_allow_html=True
        )
